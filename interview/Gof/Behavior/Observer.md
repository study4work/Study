...
Суть паттерна: Определение зависимости «один ко многим» между объектами. При изменении состояния одного объекта все зависимые объекты автоматически уведомляются.

**Как это сделано в Spring**:
Spring имеет встроенный механизм обработки событий (Event-driven architecture).  
Subject (Наблюдаемый) использует ApplicationEventPublisher для публикации событий.  
Observers (Наблюдатели) используют аннотацию @EventListener для реакции на события.  
**Пример**: Регистрация пользователя
Когда пользователь регистрируется, нам нужно отправить приветственное письмо и создать запись в журнале аудита.
